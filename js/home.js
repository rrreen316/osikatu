      ...
      <section class="section">
        ...
        ${saved.length === 0
          ...
          : saved.slice(0,3).map(p=>proposalCard(p)).join('') +
            (saved.length > 3 ? `<div style="text-align:center;margin-top:10px;"><button class="btn btn-secondary btn-block" data-page="proposals">すべて見る（${saved.length}件）→</button></div>` : '')
        }
      </section>
      <section class="section">
        <div class="section-header">
        ...
        ${groups.length === 0
          ? `<div class="empty-state"><p>グッズがまだ登録されていません</p><button class="btn btn-primary" data-page="register">グッズを登録</button></div>`
          : `<div class="goods-summary">${groups.map(g=>`
              <div class="goods-group">
                <div class="goods-group-header"><span style="font-weight:700;">${g.name}</span><span class="goods-group-count">計${g.count}個</span></div>
...
function proposalCard(p) {
  ...
  return `<div class="proposal-card">
    ...
    <div class="proposal-actions">
      ${p.isAdopted
        ? `<button class="btn btn-secondary" style="color:var(--error);border-color:var(--error);" onclick="proposalUnadopt('${k}')">採用解除</button>`
        : `<button class="btn btn-primary" onclick="proposalAdopt('${k}')">✅ 採用する</button>`
      }
      <button class="btn btn-secondary" onclick="proposalView('${k}')">詳細</button>
      <button class="btn btn-secondary" onclick="proposalRemove('${k}')">削除</button>
    </div>
  </div>`;
}
...
function proposalAdopt(key) {
  ...
  showToast('採用しました🎉'); renderCurrentProposalPage();
}
function proposalUnadopt(key) {
  const p = _cache.get(key); if (!p) return;
  const saved = DB.saveProposal({...p, isAdopted:false});
  _cache.set(key, saved);
  showToast('採用を解除しました'); renderCurrentProposalPage();
}
function proposalView(key) {
  ...
  showModal(`
    <div style="display:flex;flex-direction:column;gap:14px;">
      ...
      ${p.description ? `<div><label style="font-weight:600;font-size:12px;color:var(--muted);">説明</label><p style="margin-top:4px;">${p.description}</p></div>` : ''}
      ...
      <div><label style="font-weight:600;font-size:12px;color:var(--muted);">使用したグッズ</label><p style="margin-top:4px;">${p.goods||'なし'}</p></div>
...
function proposalRemove(key) {
  ...
  showToast('削除しました'); renderCurrentProposalPage();
}
