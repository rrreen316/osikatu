function renderGalleryPage() {
  ...
  // 投稿ボタン：data-page委譲に頼らずaddEventListenerで直接登録
  document.getElementById('goto-post-btn').addEventListener('click', function(e) {
    e.preventDefault();
    e.stopPropagation();
  ...
  document.getElementById('gallery-sort').addEventListener('change', e => {
    ...
    if (e.target.value === 'smallest')
      ...
      ps.sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt));
  ...
  return posts.map(p => {
    ...
    return `<div class="gallery-item" data-action="gallery-open" data-key="${k}" style="cursor:pointer;">
      ${p.imageUri
        ...
        : `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:#999;font-size:12px;padding:8px;text-align:center;pointer-events:none;">${p.title}</div>`}
