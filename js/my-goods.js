...
function renderMyGoodsPage() {
  ...
  const map = new Map();
  all.forEach(g => { if (!map.has(g.groupName)) map.set(g.groupName,[]); map.get(g.groupName).push(g); });
  page.innerHTML = `
          ...
          : Array.from(map.entries()).map(([gn, items]) => `
            <div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:12px;">
              ...
              <div style="display:flex;flex-direction:column;gap:8px;">
                ${items.map(g => {
                  const k = 'g_'+g.id; _goodsCache.set(k, g);
                  return `<div class="goods-card" onclick="openGoodsDetail('${k}')">
                    <div style="display:flex;justify-content:space-between;align-items:center;">
                      <div><div style="font-weight:600;font-size:14px;">${g.genre}</div>
                        <div style="font-size:12px;color:var(--muted);margin-top:2px;">${formatDate(g.createdAt)}</div></div>
                      <span class="goods-card-genre">${g.genre}</span>
                    </div>
                    ${g.memo ? `<div class="goods-card-memo"><p>${g.memo}</p></div>` : ''}
                  </div>`;
                }).join('')}
              </div>
            </div>`).join('')
...
function openGoodsDetail(key) {
  ...
  showModal(`
    <div style="display:flex;flex-direction:column;gap:14px;">
      <div><label style="font-weight:600;font-size:12px;color:var(--muted);">グループ</label><p style="margin-top:4px;">${g.groupName}</p></div>
      <div><label style="font-weight:600;font-size:12px;color:var(--muted);">ジャンル</label><p style="margin-top:4px;">${g.genre}</p></div>
      ...
      ${g.memo ? `<div><label style="font-weight:600;font-size:12px;color:var(--muted);">メモ</label><p style="margin-top:4px;">${g.memo}</p></div>` : ''}
      <div style="display:flex;gap:10px;margin-top:8px;">
        <button class="btn btn-secondary" style="flex:1;" onclick="closeModal();editGoods('${key}')">編集</button>
        <button class="btn btn-secondary" style="flex:1;background:#ef4444;color:#fff;" onclick="deleteGoods('${g.id}')">削除</button>
      </div>
    </div>`, `${g.groupName} - ${g.genre}`);
}
function deleteGoods(id) {
  if (!confirm('このグッズを削除しますか？')) return;
  DB.deleteGoods(id); closeModal(); renderMyGoodsPage();
}
